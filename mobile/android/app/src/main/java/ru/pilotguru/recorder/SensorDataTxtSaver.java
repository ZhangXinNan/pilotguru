...
public class SensorDataTxtSaver extends CameraCaptureSession.CaptureCallback implements
...
//  private BufferedWriter rotationsWriter = null, accelerationsWriter = null, locationsWriter = null,
//        framesWriter = null, elm327Writer = null, pressuresWriter = null;
  private BufferedWriter rotationsWriter = null, accelerationsWriter = null, framesWriter = null;
//  private List<String> jsonFiles = new LinkedList<>();
  private List<String> txtFiles = new LinkedList<>();
...
//  private final Lock elm327Lock = recordingStatusLock.readLock();
  ...
  public SensorDataTxtSaver(@NonNull Activity parentActivity) {
  ...
  public void start(@NonNull File recordingDir, @NonNull TextView textViewFps, @NonNull TextView textViewCamera,
    ...
    try {
      ...
      if (isRecording) {
        throw new AssertionError("Called start() but SensorDataTxtSaver is already recording.");
      }
      isRecording = true;
      ...
      switch (cameraTimestampSource) {
        case CameraMetadata.SENSOR_INFO_TIMESTAMP_SOURCE_UNKNOWN:
          ...
          // function calls.
          Log.i("SensorDataTxtSaver", "elapsedRealtimeNanos - nanoTime difference values: " +
      ...
      rotationsWriter = initTxtListWriter(recordingDir, ROTATIONS);
      accelerationsWriter = initTxtListWriter(recordingDir, ACCELERATIONS);
//      locationsWriter = initTxtListWriter(recordingDir, LOCATIONS);
      framesWriter = initTxtListWriter(recordingDir, FRAMES);
//      pressuresWriter = initTxtListWriter(recordingDir, PRESSURES);
//      elm327Writer = initTxtListWriter(recordingDir, CAN_FRAMES);
    } finally {
      recordingStatusChangeLock.unlock();
    }
  }

  private BufferedWriter initTxtListWriter(@NonNull File recordingDir, @NonNull String name) {
    BufferedWriter writer = null;
    try {
      // Init the file IO stuff.
      final File outFile = new File(recordingDir, name + ".txt");
      ...
      txtFiles.add(outFile.toString());
      writer = new BufferedWriter(new FileWriter(outFile));

    } catch (IOException e) {
      Errors.dieOnException(parentActivity, e,
              "Error trying to initialize " + name + " TXT writer.");
    }
    return writer;
  }

  private void finishTxtListWriter(@NonNull BufferedWriter writer, @NonNull String name) {
    try {
      writer.close();
    } catch (IOException e) {
      Errors.dieOnException(parentActivity, e, "Error trying to close " + name + "Txt writer.");
    }
  }
  ...
  public void stop(Context context) {
    recordingStatusChangeLock.lock();
    if (!isRecording) {
      throw new AssertionError("Called stop() but SensorDataTxtSaver is not recording.");
    }
    isRecording = false;

    finishTxtListWriter(rotationsWriter, ROTATIONS);
    finishTxtListWriter(accelerationsWriter, ACCELERATIONS);
//    finishTxtListWriter(locationsWriter, LOCATIONS);
//    finishTxtListWriter(pressuresWriter, PRESSURES);
    finishTxtListWriter(framesWriter, FRAMES);
//    finishTxtListWriter(elm327Writer, CAN_FRAMES);
    MediaScannerConnection.scanFile(context, txtFiles.toArray(new String[0]), null, null);
    txtFiles.clear();
  ...
  private void writeNamedSensorValues(
      Lock lock, SensorEvent event, String[] valueNames, BufferedWriter writer, String sensorName) {
    try {
      lock.lock();
      if (isRecording) {
        String content = Long.toString(event.timestamp);
//        writer.beginObject();
...
//        writer.endObject();
      ...
      Errors.dieOnException(parentActivity, e, "Error writing " + sensorName + " TXT.");
    } finally {
      lock.unlock();
    }
  }
...
//      }
//    } catch (IOException e) {
//      Errors.dieOnException(parentActivity, e, "Error writing accelerations TXT.");
//    } finally {
//      locationLock.unlock();
//    }
  }
...
//        framesWriter.endObject();
        framesWriter.write(timeUsec + " " + currentFrameNumberInSequence + "\n");
      ...
      Errors.dieOnException(parentActivity, e, "Error writing frames timestamps txt.");
    } finally {
